...
export const funcElev = (numbers) => {
    let newNumbers = []
    !isNaN(numbers) === true || numbers.length === 0 || !Array.isArray(numbers)
        ? console.log('Inserta varios numeros diferentes dentro de un array')
        : numbers.forEach(element => {
            newNumbers.push((Math.pow(element, 2)))
        });

    if (newNumbers.length != 0) console.log(newNumbers)
}
// funcElev([1, 4, 5])
...
export const minMax = (numbers) => {
    ...
    !isNaN(numbers) === true || numbers.length === 0 || !Array.isArray(numbers)
        ? console.log(console.log('Inserta varios numeros diferentes dentro de un array'))
        : console.log([min, max])
}
// minMax([1, 4, 5, 99, -60])
